...
const getCompletedStudents = (course: Course): Student[] =>
  course.students.filter((a) => a.hasCompleted);
...
const printCourseReport = (courseManager: CourseManager) => {
  courseManager.courses.map((target, index) => {
    console.log(
      index + ". Khoa: " + target.title + ` (GV: ${target.instructor})`
    );
    ...
    console.log(
      "     - Trang thai: " + (target.isActive ? "DANG MO" : "DA DONG")
    );
  });
};
